...
try:
    ...
    # Si falla, intentar con otras localizaciones comunes en español
    try:
        ...
        try:
            ...
            pass  # Continuamos aunque falle la configuración del locale
...
def traducir_mes(fecha):
    try:
        ...
        return f"{fecha.day} de {nombre_mes} de {fecha.year}"
    except:
        return fecha.strftime('%Y-%m-%d')  # Fallback en caso de error

# Interfaz gráfica con Streamlit
st.set_page_config(layout="wide", page_title="NASA POWER Data Explorer")
st.title("Interfaz NASA POWER")

# Configuración para matplotlib - usar fuente con soporte para caracteres españoles
...
with st.sidebar:
    ...
    st.caption(f"Periodo seleccionado: {traducir_mes(fecha_inicio)} - {traducir_mes(fecha_fin)}")
...
if not st.session_state.resultado_df.empty:
    ...
    # Convertir el índice de fecha al formato español
    df_display.index = [traducir_mes(fecha) for fecha in df_display.index]
    ...
    # Configurar matplotlib para usar fuentes con caracteres españoles
    plt.rcParams['font.family'] = 'DejaVu Sans'
    ...
    with col1:
        csv = st.session_state.resultado_df.reset_index().to_csv(index=False).encode('utf-8')
        fecha_inicio_str = fecha_inicio.strftime('%d_%m_%Y')
        fecha_fin_str = fecha_fin.strftime('%d_%m_%Y')
    ...
    if stats_df is not None:
        ...
        # Visualización de estadísticas
            ...
            # Configurar matplotlib para usar fuentes con soporte para caracteres españoles
            plt.rcParams.update({
                'font.family': 'DejaVu Sans',
                'axes.unicode_minus': False  # Para mostrar correctamente los símbolos negativos
            })
            ...
            for i, col in enumerate(st.session_state.resultado_df.columns):
                with tabs[i]:
                    ...
                    if len(st.session_state.resultado_df) > 28:
                        ...
                        # Convertir números de mes a nombres en español
                        meses_es = ["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", 
                                   "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"]
                        monthly_avg['month_name'] = monthly_avg['month'].apply(lambda x: meses_es[x-1])
                        
                        # Mostrar promedios mensuales
                        monthly_fig, monthly_ax = plt.subplots(figsize=(10, 5))
                        monthly_ax.bar(monthly_avg['month_name'], monthly_avg[col], color='skyblue')
                        monthly_ax.set_title(f'Promedio mensual - {rename_dict.get(col, col)}')
                        monthly_ax.set_xlabel('Mes')
                        monthly_ax.set_ylabel('Valor promedio')
                        plt.xticks(rotation=45)
